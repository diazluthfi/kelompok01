<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Tourism Surabaya</title>

    <!-- font awesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.3.0/css/all.min.css">

    <!-- css -->
    <link rel="stylesheet" href="assets/css/style.css">
</head>
<body>
    <!-- header starts -->

    <header class="header">

        <nav class="navbar">
    
        <a href="index.html" class="logo">
            <img src="assets/images/logo/img-logo-sparkling.png" alt="">
        </a>
    
        <div class="links">
            <a href="{{route('home')}}">Beranda</a>
          <a href="{{route('pagedestinasi')}}">Destinasi</a>
          <a href="kuliner.html">Kuliner</a>
          <a href="{{route('login')}}">Login</a>
          <a href="{{route('register')}}">Sign Up</a>
            
        </div>
    
        <div id="menu-btn" class="fa fa-bars"></div>
    
        </nav>
    
    </header>

    <!-- header ends -->

    

    <!-- destinasi starts -->

    <section class="destination" id="destination">

        <h1 class="heading">Destinasi <span>Wisata</span></h1>

        <!-- accordion filter starts -->
        <div class="filter-container">
            <div class="accordion-box">
                <button class="accordion">Filter By Region</button>
                <div class="panel">
                   
                    <label><input type="checkbox" name="region" value="asia">Asia</label>
                    <label><input type="checkbox" name="region" value="europe">Europe</label>
                    <label><input type="checkbox" name="region" value="america">America</label>
                    <!-- <button class="region-filter">Filter Region</button> -->
                </div>
            </div>
            <div class="accordion-box">
                <button class="accordion">Filter By Category</button>
                <div class="panel">
                    <label><input type="checkbox" name="category" value="food">Food</label>
                    <label><input type="checkbox" name="category" value="drink">Drink</label>
                    <!-- <button class="category-filter">Filter Category</button> -->
                </div>
            </div>
            <div class="accordion-box">
                <button class="accordion">Filter By Name</button>
                <div class="panel">
                    <input type="text" name="name" placeholder="Search by name">
                    <!-- <button class="name-filter">Filter Name</button> -->
                </div>
            </div>
            <button class="filter">Filter</button>
        </div>
        <!-- accordion filter ends -->
        <div class="tag" style="display: flex;flex-wrap:wrap; justify-content: center; margin-top: 4rem; ">
        @foreach ($destinations as $destinasi)

        <div class="box-container">
            <div class="box">
                <div class="image" >
                    <img src="assets/images/news/news-4.jpg" alt="">
                </div>
                <div class="content" >
                    <h3>{{$destinasi->name}}</h3>
                    <p>{{$destinasi->address}}</p>
                    <a href="#" class="btn">baca sekarang</a>
                </div>
            </div>
                
    
        </div>

        @endforeach
    </div>
    

    </section>

    <!-- destinasi ends -->

    

    <!-- footer section starts -->
  <footer class="footer">

    <section class="box-container">

      <div class="box">
        <h3>Call to Action Links</h3>
        <a href="index.html">beranda</a>
        <a href="destinasi.html">destinasi</a>
        <a href="kuliner.html">kuliner</a>
        <a href="login.html">login</a>
        <a href="signup.html">sign up</a>
      </div>

      <div class="box">
        <h3>Contact Us</h3>
        <a href="#"></i>031-5318409</a>
        <a href="map">Jl. Tunjungan No.1-3 Kelurahan Genteng
            Kecamatan Genteng, Kota Surabaya
            Jawa Timur, Indonesia, 60275</a>
      </div>

      <div class="box">
        <h3>Social Media</h3>
        <a href="#">facebook</a>
        <a href="#">instagram</a>
      </div>

    </section>

    <div class="credit"> 
        <img src="assets/images/logo/img-logo-sparkling.png" alt="">
    </div>
  </footer>
  <!-- footer section ends -->
    








    <!-- JS -->
    <script src="assets/js/app.js"></script>
    
</body>
</html>